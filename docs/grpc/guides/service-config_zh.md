...
本页概述了服务配置中的选项，但完整的服务配置数据结构是用 [protobuf 定义](https://github.com/grpc/grpc-proto/blob/master/grpc/service_config/service_config.proto) 记录的。
...
- 启用[等待就绪](https://grpc.io/docs/guides/wait-for-ready/)后，如果客户端无法连接到后端，
RPC 将被延迟而不是立即失败。
- 可以提供一个调用[超时](https://grpc.io/docs/guides/deadlines/)，表示客户端的最大时间
在放弃 RPC 之前应该等待。
- 以下之一：
- [重试](https://grpc.io/docs/guides/retry/)策略（最大尝试次数、退避设置、可重试状态码）
- [请求对冲](https://grpc.io/docs/guides/request-hedging/)策略（最大尝试次数、延迟、非致命状态码）
这些调用行为设置可以限制为单个服务或方法。

可以通过设置“重试限制策略”来进一步调整重试和请求对冲策略，但它将适用于所有服务和方法。
#### 健康检查

可以将客户端配置为通过提供运行状况检查名称来执行[健康检查](https://grpc.io/docs/guides/health-checking/)。然后客户端将使用标准的 gRPC 健康检查服务。
...
gRPC [名称解析机制](https://github.com/grpc/grpc/blob/master/doc/naming.md) 允许可插入名称解析器实现。这些实现返回与名称关联的地址以及关联的服务配置。服务所有者可以使用这种机制将其服务配置分发给一组 gRPC 客户端。

- xDS 名称解析器转换从
控制平面到相应的服务配置。
- Go实现中的标准DNS名称解析器支持服务
名称服务端上的配置[存储为 TXT 记录](https://github.com/grpc/proposal/blob/master/A2-service-configs-in-dns.md)。
